...
public class SwingClient {
  ...
  class newPublisherHandler implements ActionListener {
    ...
    @Override
    public void actionPerformed(ActionEvent e) {
        /* Backend - Non Graphical Part */
        // Get topic from textfield and create topic
        Topic topic = new Topic(argument_TextField.getText());
        // Add the topic and create publisher if does not have a publisher or exist
        if (topicManager.isTopic(topic).isOpen) return;
        publisher = topicManager.addPublisherToTopic(topic);
      
        /* Frontend - Graphical Part */
        // Clear all text
        topic_list_TextArea.setText(null);
        // Put current topics in the topicList
        for (Topic t : topicManager.topics())
            topic_list_TextArea.append(t.name);
        // tell you where you are publishing
